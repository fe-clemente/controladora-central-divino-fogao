...
function lerCache() {
    try {
        ...
        console.log(`  [SULTS CACHE][JUR] Carregado — ${json.dados?.totalUnidades} unidades, ${json.dados?.totalFuncionarios} funcionários`);
        console.log(`  [SULTS CACHE][JUR] Última sync: ${json.sincronizadoEm}`);
        return json;
    } catch (e) {
        console.warn('  [SULTS CACHE][JUR] Falha ao ler:', e.message);
        return null;
    }
}
...
function salvarCache(dados, fingerprint) {
    try {
        ...
        console.log(`  [SULTS CACHE][JUR] Salvo — ${dados.totalUnidades} unidades, ${dados.totalFuncionarios} funcionários`);
    } catch (e) {
        console.warn('  [SULTS CACHE][JUR] Falha ao salvar:', e.message);
    }
}
...
async function sincronizarEAtualizar(motivo = '') {
    if (syncEmAndamento) {
        console.log('  [SULTS CACHE][JUR] Sync já em andamento, aguardando...');
        while (syncEmAndamento) await new Promise(r => setTimeout(r, 500));
        return dadosEmMemoria;
    }

    syncEmAndamento = true;
    console.log(`🔄 [SULTS][JUR] Iniciando sync completo${motivo ? ' — ' + motivo : ''}...`);
    ...
    try {
        ...
        console.log(`✅ [SULTS][JUR] Sync concluído — ${dados.totalUnidades} unidades, ${dados.totalFuncionarios} funcionários`);
        return dados;
    } catch (e) {
        console.error('❌ [SULTS][JUR] Sync falhou:', e.message);
        throw e;
    } finally {
...
async function verificarMudancas() {
    ...
    if (idadeCache > CACHE_MAX_AGE) {
        console.log('  [SULTS CACHE][JUR] Cache com mais de 24h — re-sincronizando...');
        await sincronizarEAtualizar('cache expirado');
    } else {
        const minutosRestantes = Math.round((CACHE_MAX_AGE - idadeCache) / 60000);
        console.log(`  [SULTS CACHE][JUR] Cache válido — próxima sync em ~${minutosRestantes}min ✓`);
...
async function inicializar() {
    console.log('⚖️ [SULTS][JUR] Inicializando cache...');
    const cache = lerCache();

    if (cache && cache.dados) {
        dadosEmMemoria    = cache.dados;
        ultimaVerificacao = Date.now();
        console.log('⚡ [SULTS][JUR] Dados carregados do cache — servidor pronto imediatamente!');

        setTimeout(async () => {
            try { await verificarMudancas(); } catch (e) {
                console.warn('  [SULTS][JUR] Verificação background falhou:', e.message);
            }
        }, 10000);
    } else {
        console.log('  [SULTS][JUR] Sem cache — fazendo sync inicial (pode demorar ~40s)...');
        try {
            await sincronizarEAtualizar('primeira inicialização');
        } catch (e) {
            console.error('❌ [SULTS][JUR] Sync inicial falhou:', e.message);
        }
    }
    ...
    checkTimer = setInterval(async () => {
        try { await verificarMudancas(); } catch (e) {
            console.warn('  [SULTS CACHE][JUR] Verificação periódica falhou:', e.message);
        }
    }, CHECK_INTERVAL);

    console.log(`⏰ [SULTS][JUR] Verificação automática a cada ${CHECK_INTERVAL / 60000} minutos`);
}
...
function getDados() {
    if (!dadosEmMemoria) {
        ...
        if (cache && cache.dados) {
            dadosEmMemoria    = cache.dados;
            ultimaVerificacao = Date.now();
            console.log('⚡ [SULTS][JUR] getDados() — recarregado do arquivo JSON automaticamente');
        }
    }
    return dadosEmMemoria;
}
