// modulos/juridico/services/uploadsCache.js
'use strict';
...
async function inicializar() {
  console.log('[JURIDICO-UPLOADS] Inicializando cache de pastas...');
...
async function sincronizarEAtualizar(origem = 'manual') {
  ...
  try {
    if (!PASTA_RAIZ_ID) {
      _status = { estado: 'erro', ultimoErro: 'JURIDICO_UPLOADS_PASTA_ID não configurado', sincronizadoEm: null };
      return null;
    }
    const pastas = await listarPastas(PASTA_RAIZ_ID);
    _dados = { pastas, totalPastas: pastas.length, sincronizadoEm: new Date().toISOString() };
    _status = { estado: 'pronto', ultimoErro: null, sincronizadoEm: _dados.sincronizadoEm, origem };
    console.log(`[JURIDICO-UPLOADS] Cache atualizado (${origem}): ${pastas.length} pasta(s)`);
    return _dados;
  } catch (e) {
    _status = { estado: 'erro', ultimoErro: e.message, sincronizadoEm: _status.sincronizadoEm };
    console.error('[JURIDICO-UPLOADS] Erro ao sincronizar cache:', e.message);
    throw e;
  }
}
